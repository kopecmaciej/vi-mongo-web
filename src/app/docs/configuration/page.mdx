...
2. `styles.yaml`: UI styling configuration
...
## Styling Configuration (styles.yaml)

The `styles.yaml` file allows you to customize the appearance of Vi Mongo. You can modify colors, symbols, and other visual elements. Here's an example of the structure:
...
## Keybindings Configuration (keybindings.json)
...
To modify a keybinding, change the `keys` or `runes` array for the desired action. Make sure to use valid key names as recognized by the application.
