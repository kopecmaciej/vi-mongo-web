...
The query bar accepts standard MongoDB query syntax, including regex literals and helper functions:
...
// Using regex with $in operator
{ "status": { "$in": [/active/i, /pending/i] } }

// Regex in arrays
{ "tags": [/mongodb/i, /database/] }
```
...
You can also use the AI Query feature (`Alt+A`) to generate queries using natural language.
...
- MongoDB types are not yet properly tested so have in mind that some types like `BSONSymbol` or `Code` can be missing or not displayed properly.
