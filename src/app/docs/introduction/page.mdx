...
- **Fast switching between databases**: Vi Mongo allows you to switch between
  databases quickly.
...
- **Managing Collections**: Vi Mongo provides a simple way to manage your
  collections, including the ability to create and delete collections.
...
- **Index Management**: Vi Mongo provides a simple way to manage your indexes,
  including the ability to create and delete indexes.
- **Query History**: Vi Mongo keeps track of your query history, allowing you to
  easily access and reuse previous queries.
- **Mongosh Syntax Support**: Vi Mongo supports standard MongoDB Shell (mongosh)
  syntax, including regex literals (`/pattern/flags`), `ISODate()`, `NumberInt()`,
  `NumberLong()`, and `NumberDecimal()` helper functions.
- **Multiple Styles**: Vi Mongo supports multiple color schemes, which can be
  selected in the config file or added/modified easily.
